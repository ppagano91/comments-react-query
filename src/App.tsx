...
import { getComments, type CommentWithId, type Comment, postComment } from './services/comments'
...
function App () {
  ...
  const { mutate, isLoading: isLoadingMutation } = useMutation({
    ...
    // 1. Actualizar el caché de la Query manualmente
    // onSuccess: async (newComment)=>{
    //   await queryClient.setQueriesData(["comments"], (oldData?:CommentWithId[]) => {
    //     if (oldData == null) return [newComment]
    //     return [...oldData, newComment]
    //   })
    // 2. Hace otra vez refetch de la query
